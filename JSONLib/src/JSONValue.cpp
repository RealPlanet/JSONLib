...
int64_t LiteralValue::as_integer() const {
	return std::stoi(m_Value);
}

double LiteralValue::as_fractional() const
{
	return std::stod(m_Value);
}

bool LiteralValue::as_bool() const {
	return m_Value == "true";
}

void LiteralValue::operator=(const char* newVal) {
...
Element* LiteralValue::copy() const {
	return new LiteralValue(m_Type, as_str());
}
