...
JValue* JValue::create_str_value(const std::string& s) {
	return new JValue(ValueType::String, s);
}

JValue* JValue::create_number_value(const double d) {
	return new JValue(ValueType::Number, std::to_string(d));
}

JValue* JValue::create_number_value(const int64_t i) {
	return new JValue(ValueType::Number, std::to_string(i));
}

JValue* JValue::create_boolean_value(const bool b) {
	return new JValue(ValueType::Boolean, b ? "true" : "false");
}

JValue* JValue::create_null_value() {
...
JElement* JValue::copy() const {
	return new JValue(m_Type, as_string());
}
