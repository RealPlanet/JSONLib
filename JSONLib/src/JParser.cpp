...
JSON json::JParser::from_iterator(DataIterator& it, std::vector<std::string>* errors)
{
	JParser p;
...
JElement* JParser::parse_json_value(DataIterator& it) {
	while (it.is_valid()) {
		...
		case '"':
		{
			...
			return JValue::create_str_value(s);
		}
		case '0':
		case '1':
		case '2':
		...
		default:
			build_error(JParser::UnexpectedCharacterInFile, it);
			it.offset = it.data.size();
			break;
		}
	}
...
JElement* JParser::parse_number(DataIterator& it) {
	...
	try {
		...
		return JValue::create_number_value((int64_t)std::stoi(number));
	}
	catch (std::out_of_range&)
	{
		return JValue::create_number_value((int64_t)0);
	}
}
...
bool JParser::get_escaped_character(DataIterator& it, std::string& escapedCharacters) {
	...
	default:
		build_error(JParser::UnexpectedEscapeCharacter, it);
	}
	return false;
}
...
void JParser::build_error(ErrType type, DataIterator& data) {
	...
	if (charIndex < data.data.size())
	{
		...
		while (endIndex < data.data.size() - 1 &&
	...
	case ErrType::MaxJSONDepthReached:
		templ = utility::string_format(templ, JParser::MaxDepth, charIndex);
		break;
	default:
